...
def _simple_authentication(security_header: str = Security(security)):
    if not security_header or not security_header.credentials:
        raise HTTPException(status_code=401, detail="Not authenticated")
    if security_header.credentials != "wazza":
...
print(settings().server.auth.enabled)
